...
def teacher_tab_take_attendance():
    ...
    if not subjects:
        st.warning("You havent created any subject yet! Pleace create one to begin!")
        return 
    ...
    with col1:
        selected_subject_lebel = st.selectbox("Select Subject", options=list(subject_options.keys()))

    with col2:
        if st.button('Add Photos', type = 'primary', icon=":material/photo_prints:", width='stretch'):
            add_photos_dialog()

    selected_subject_id = subject_options[selected_subject_lebel]
        ...
        if st.button('Run Face Analysis', width="stretch", type='secondary', icon= ":material/analytics:", disabled=not has_photos) :
                ...
                with st.spinner('Deep scanning classroom photos....'):
                    ...
                    if not enrolled_students:
                        ...
                        currect_timestamp = datetime.now().strftime("%Y-%m-%d T %H:%M:%S")
...
def register_teacher(teacher_username,teacher_name,teacher_pass,teacher_pass_confirm):
    if not teacher_username or not teacher_name or not teacher_pass:
        return False, "All Fields are requiered!"
